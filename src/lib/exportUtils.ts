...
function generateOrdersData(orders: OrderData[]) {
  ...
  return completedOrders.map(order => ({
    ...
    'Total (USD)': order.total_usd?.toFixed(2) || '0.00',
    'Status': order.status,
    'Notes': order.customer_notes || '',
  }));
}

function generateRevenueSummaryData(orders: OrderData[], dateRange: DateRange) {
  ...
  return days.map(day => {
    ...
    return {
      ...
      'Revenue (USD)': data.revenue.toFixed(2),
      'Avg Order Value (USD)': data.orderCount > 0 
...
export function exportData({ orders, dateRange, type, format: exportFormat }: ExportOptions) {
  const data = type === 'orders' 
    ? generateOrdersData(orders)
    : generateRevenueSummaryData(orders, dateRange);
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, type === 'orders' ? 'Orders' : 'Revenue Summary');
  ...
  const fileName = `${type === 'orders' ? 'orders' : 'revenue-summary'}_${dateStr}.${exportFormat}`;
