...
const ReceiptSummary = ({ session, subtotal, formatPrice }: ReceiptSummaryProps) => {
  const taxRate = session.default_tax_percentage || 0;
  const serviceRate = session.service_charge_percentage || 0;

  // Use frozen rate for paid sessions, current rate for unpaid
  ...
  return (
    <div className="my-4">
      ...
      <div className="space-y-1.5 text-sm">
        ...
        {serviceRate > 0 && session.show_service_charge_on_receipt && (
          <div className="flex justify-between" style={{ color: '#888' }}>
